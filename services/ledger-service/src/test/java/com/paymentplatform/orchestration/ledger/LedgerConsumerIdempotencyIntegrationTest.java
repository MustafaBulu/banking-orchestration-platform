...
class LedgerConsumerIdempotencyIntegrationTest extends AbstractIntegrationTest {
    ...
    @Test
    void deliversTwiceButPostsLedgerEntriesOnce() throws Exception {
        createTopic();
        ...
        String sentinel = envelope(sentinelId, UUID.randomUUID().toString(), "customer-2", "10.00", "EUR");
        ...
        Integer distinctLines = jdbcTemplate.queryForObject(
                "SELECT COUNT(DISTINCT line_number) FROM ledger_entries WHERE source_event_id = ?",
                Integer.class, eventId);
        assertThat(distinctLines).isEqualTo(2);
